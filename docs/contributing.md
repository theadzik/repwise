...
| A new set of weights | A named entry under the top-level `weights`, with its `min`, `step` and any `max`. An exercise's `load` must name one of them, or loading rejects it |
